...
def add_buzzer_params(parser):
    parser.add_argument('--buzzer_guessers', nargs='+', default=['Tfidf'], help='Guessers to feed into Buzzer',
                        type=str)
    parser.add_argument('--buzzer_history_length', type=int, default=0,
                        help="How many time steps to retain guesser history")
    parser.add_argument('--buzzer_history_depth', type=int, default=0,
                        help="How many old guesses per time step to keep")
    parser.add_argument('--features', nargs='+', help='Features to feed into Buzzer', type=str,
                        default=["Length", "Keyword", "Confidence", "Specificity", "Temporal"])
...
def add_guesser_params(parser):
    ...
    parser.add_argument('--guesser_min_length', type=int,
                        help="How long (in characters) must text be before it is indexed?", default=50)
    ...
    parser.add_argument('--guesser_max_length', type=int, help="How long (in characters) must text be to be removed?",
                        default=500)
    parser.add_argument('--guesser_split_sentence', type=bool, default=True,
                        help="Index sentences rather than paragraphs")
    parser.add_argument('--wiki_min_frequency', type=int,
                        help="How often must wiki page be an answer before it is used", default=10)
    ...
    if flags.question_source == 'json':
        with open(question_filename) as infile:
            try:
                ...
                logging.error(
                    "Got a Unicode decode error while reading json questions.  This can mean: 1) your data are corrupt (redownload them), 2) you're trying to use json question source on 'gzjson' type data (change the question_source flag)")
...
def instantiate_guesser(guesser_type, flags, load):
    ...
    if guesser_type == "Dan":
        ...
        guesser = DanGuesser(filename=flags.DanGuesser_filename, answer_field=flags.guesser_answer_field,
                             min_token_df=flags.DanGuesser_min_df, max_token_df=flags.DanGuesser_max_df,
...
def load_buzzer(flags, load=False):
    ...
    for gg in flags.buzzer_guessers:
        ...
        if primary:
            primary_loaded += 1
    assert primary_loaded == 1 or (
                primary_loaded == 0 and flags.primary_guesser == 'consensus'), "There must be one primary guesser"
    ...
    for ff in flags.features:
        ...
        if ff == "Keyword":
            ...
            feature = KeywordFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)


        if ff == "Confidence":
            from features import ConfidenceTimingFeature
            feature = ConfidenceTimingFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)

        if ff == "Specificity":
            from features import AnswerSpecificityFeature
            feature = AnswerSpecificityFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)

        if ff == "Temporal":
            from features import TemporalDynamicsFeature
            feature = TemporalDynamicsFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
